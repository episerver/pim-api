...
public class CategoryTaxonomyQueryTests
{
    ...
    public async Task ShouldResolveAndDeserializeQueries(string serializerKey)
    {
        ...
        var entities = await result.GetDataAsync<ODataResponseCollection<CategoryTaxonomyDto>>(
            jsonSerializer
        );
        entities.Should().NotBeNull();

        var queryById = new GetByCategoryTaxonomyId { Id = entities.Value.FirstOrDefault()!.Id };

        var entity = await queryById.GetEntityById<GetByCategoryTaxonomyId, CategoryTaxonomyDto>(
            jsonSerializer
        );
