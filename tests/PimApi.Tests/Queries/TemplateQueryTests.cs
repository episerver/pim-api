...
namespace PimApi.Tests.Queries
{
    ...
    public class TemplateQueryTests
    {
        ...
        public async Task ShouldResolveAndDeserializeQueries
            (string serializerKey)
        {
            var jsonSerializer = serializerKey.GetJsonSerializer();
            var query = new GetTemplates
            {
                Skip = 0,
                Top = 10,
            };
            ...
            var templates = await result.GetDataAsync<ODataResponseCollection<TemplateDto>>(jsonSerializer);
            templates.Should().NotBeNull();

            var queryById = new GetByTemplateId
            {
                Id = templates.Value.FirstOrDefault()!.Id
            };
