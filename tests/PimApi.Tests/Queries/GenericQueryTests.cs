...
public class GenericQueryTests
{
    ...
    [TestCase(SystemTextJsonSerializer)]
    [TestCase(NewtonsoftJsonSerializer)]
    public async Task ShouldResolveAndDeserializeCustomQueryWithInvalidRendererChoice(
        string serializerKey
    )
    {
        var jsonSerializer = serializerKey.GetJsonSerializer();
        var renderer = new UserChoiceRenderer(
            [new EntityListRenderer(), new TemplateDetailRenderer(), new TemplateListRenderer()]
        )
        {
            ChosenRenderer = 1000
        };

        var query = new GenericQuery(renderer) { QueryText = "products?$top=1", };
