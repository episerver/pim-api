...
namespace PimApi.Tests.Queries
{
    ...
    public class PropertyQueryTests
    {
        ...
        public async Task GetPropertyGroupsShouldResolveAndDeserialize(string serializerKey)
        {
            ...
            var entities = await result.GetDataAsync<ODataResponseCollection<PropertyGroupDto>>(jsonSerializer);
        ...
        public async Task ShouldResolveAndDeserializeQueries(string serializerKey)
        {
            ...
            await query.ShouldRenderMessage(result, jsonSerializer);
            var entities = await result.GetDataAsync<ODataResponseCollection<PropertyDto>>(jsonSerializer);
            entities.Should().NotBeNull();

            var queryById = new GetByPropertyId
            {
        ...
        {
            var jsonSerializer = serializerKey.GetJsonSerializer();
            var query = new SearchPropertiesMappedToField
            {
                MappedToField = "_Attribute"
            };
        ...
        {
            var jsonSerializer = serializerKey.GetJsonSerializer();
            var query = new SearchPropertiesByNameInList
            {
                PropertyNames = new[] { "productNumber" }
            };
