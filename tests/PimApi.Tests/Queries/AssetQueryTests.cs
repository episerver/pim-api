...
public class AssetQueryTests
{
    ...
    {
        var jsonSerializer = serializerKey.GetJsonSerializer();
        var query = new GetAssets { Skip = 0, Top = 10, };
        ...
        var queryById = new GetByAssetId { Id = entities.Value.FirstOrDefault()!.Id };

        var entity = await queryById.GetEntityById<GetByAssetId, AssetDto>(jsonSerializer);
