...
namespace PimApi.Extensions
{
    public static class TypeExtensions
    {
        ...
        private static readonly Dictionary<Type, Dictionary<string, PropertyInfo>> TypeProperties = new(50);

        public static bool CanBeODataResponse<TEntity>(this Type typeToConvert)
        {
            if (!typeToConvert.IsGenericType) { return false; }
            if (!typeof(ODataResponse).IsAssignableFrom(typeToConvert.GetGenericTypeDefinition())) { return false; }
        ...
        /// <summary>
        /// Gets all types deriving from <see cref="BaseEntityDto.cs"/> in the assembly where it exists
        /// </summary>
        /// <param name="_"></param>
        /// <returns></returns>
        public static List<Type> GetEntityTypes(this IJsonSerializer _)
        {
            if (entityTypes.Count > 0) { return entityTypes; }

            foreach (var type in typeof(BaseEntityDto).Assembly.GetTypes())
            {
                if (!typeof(BaseEntityDto).IsAssignableFrom(type)
                    || type.IsAbstract
                    || type.GetConstructors().All(o => o.GetParameters().Length != 0)) { continue; }
        ...
        /// <summary>
        /// Gets case insensitive dictionary of type properties for JSON converter
        /// </summary>
        /// <param name="t"></param>
        /// <returns></returns>
        public static Dictionary<string, PropertyInfo> GetTypeProperties(this Type t) => TypeProperties.TryGetValue(t, out var properties)
                ? properties
                : (TypeProperties[t] = properties = t
                    .GetProperties()
