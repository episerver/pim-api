...
public static class QueryExtensions
{
    public static T GetValueOrFallback<T>(this string? s, T fallback)
    {
        ...
        if (fallback is string && s is object ss)
        {
            return (T)ss;
        }
        ...
        try
        {
            return (T)Convert.ChangeType(s.Trim(), typeof(T));
        }
        catch
        {
            return fallback;
    ...
    public static string GetPreviousDate(
        ...
        int previousDays
    ) => dateTimeProvider.GetUtcNow().Date.AddDays(-1 * Math.Abs(previousDays)).GetQueryFormat();

    public static bool IsYes(this string? s) =>
        s?.Length > 0 && (s[0].Equals('y') || s[0].Equals('Y'));

    public static string GetValueToSearch(this string? valueToSearch) =>
        valueToSearch is null || valueToSearch == "null" ? "null" : $"'{valueToSearch}'";
