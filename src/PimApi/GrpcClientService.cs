...
namespace PimApi
{
    public class GrpcClientService 
    {
        ...
        public async Task<ImportProductResponse> Import(string importTemplate, string timeZone, IReadOnlyCollection<Product> products)
        {           
        ...
        public async Task<GetImportStatusResponse> GetImportStatus(string requestId)
        {
            ...
            return await client.GetImportStatusAsync(new GetImportStatusRequest { RequestId = requestId }, metadata);
        }

        private static Metadata GenerateMetadata(string methodName, string turnstileKey, string turnstileSecret)
        {
            ...
            var message = $"{turnstileKey}{method.ToString().ToUpper()}{path}{timestamp}{nonce}{hashBody}";
