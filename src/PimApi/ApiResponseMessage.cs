...
public class ApiResponseMessage(Task<HttpResponseMessage> apiResponse) : IDisposable
{
    private object? data;
    ...
    public Task<HttpResponseMessage> GetHttpResponseMessage() => apiResponse;
        ...
        where TData : class
    {
        ...
        {
            throw new Exception(
                $"Unable to get data from response due to status code: {(int)apiResponse.Result.StatusCode}"
            );
        }
    ...
    public static implicit operator ApiResponseMessage(Task<HttpResponseMessage> message) =>
        new(message);
}
