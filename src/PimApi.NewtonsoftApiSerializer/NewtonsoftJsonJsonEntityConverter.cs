    ...
    where TEntity : BaseEntityDto, new()
{
    ...
    )
    {
        ...
        while (reader.Read())
        {
            ...
            {
                throw new Exception("Unable to read propertyName");
            }

            typeProperties.TryGetValue(propertyName, out var propertyInfo);

            if (propertyInfo is not null && !propertyInfo.PropertyType.IsPrimitive)
            {
                reader.Read();
                propertyInfo.SetValue(
                    data,
                    serializer.Deserialize(reader, propertyInfo.PropertyType)
                );
            ...
            switch (reader.TokenType)
            {
                ...
                case JsonToken.Boolean:
                    propertyValue = stringValue is string s ? Convert.ToBoolean(s) : null;
                    break;
                ...
                case JsonToken.Integer:
                    propertyValue = stringValue is not string ? null : Convert.ToInt32(stringValue);
                    break;
