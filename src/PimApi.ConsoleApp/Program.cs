...
namespace PimApi.ConsoleApp
{
    ...
    public partial class Program
    {
            ...
            while (!isStop)
            {
                ...
                switch (selectedValue)
                {
                    case "1":
                        ...
                        var apiClient = await CreateAuthorizedPimApiClientFromFile(fullConnectionFilePath, jsonSerializer);

                        await Execute(apiClient, jsonSerializer, GetQueries());
                        break;
                    case "2":
                        var connectionInformation = await GetConnectionInformation(fullConnectionFilePath, jsonSerializer);
                        await ShowGrpcServices(jsonSerializer, connectionInformation?.PimUrlBase, connectionInformation?.AppKey, connectionInformation?.AppSecret);
                        break;
                    case "3":
                        return;
                    default:
        ...
        private static async Task<ConnectionInformation> GetConnectionInformation(string fullFilePath, IJsonSerializer jsonSerializer)
        {
