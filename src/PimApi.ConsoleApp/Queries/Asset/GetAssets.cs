...
namespace PimApi.ConsoleApp.Queries.Asset
{
    ...
    public class GetAssets : IQuery, IQueryWithMessageRenderer, IQueryWithTopSkip
    {
        ...
        public ApiResponseMessage Execute(HttpClient pimApiClient)
        {
            ...
            {
                imageFilter = Program.ReadValue("Please enter asset type:", "image", types).StartsWith("i", StringComparison.OrdinalIgnoreCase);
            }

            if (externalOnly is null)
            {
                externalOnly = Program.ReadValue("External assets only?", "no", Program.YesNoChoice).IsYes();
            }
            ...
            var query = new ODataQuery<AssetDto>
            {
                ...
                Filter = $"assettype eq '{GetImageFilter(imageFilter)}' and externalfileurl {GetExternalOnly(externalOnly)}"
            };

            return pimApiClient.GetAsync(query);
        }

        private static string GetImageFilter(bool? imageFilter) => imageFilter is true
            ? "image"
            : "other";

        private static string GetExternalOnly(bool? externalOnly) => externalOnly is true
            ? "ne ''"
            : "eq ''";
    }
}