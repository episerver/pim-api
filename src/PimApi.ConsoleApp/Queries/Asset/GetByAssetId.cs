...
namespace PimApi.ConsoleApp.Queries.Asset
{
    ...
    {
        IApiResponseMessageRenderer IQueryWithMessageRenderer.MessageRenderer => AssetDetailRenderer.Default;

        public Guid? Id { get; set; }

        public ApiResponseMessage Execute(HttpClient pimApiClient) =>
            pimApiClient.GetAsync(new ODataQuery<AssetDto>
            {
                Id = this.GetIdValue(),
                Expand = $"{nameof(AssetDto.ProductAssets)}($expand={nameof(ProductAssetDto.Product)})"
            });
    }
}