...
namespace PimApi.ConsoleApp.Queries.Product
{
    ...
    public class SearchByDateFieldLastNDays : IQuery, IQueryWithMessageRenderer, IQueryWithTopSkip
    {
        ...
        public SearchByDateFieldLastNDays() : this(DateTimeProvider.Default) { }
        ...
        {
            var previousDays = this.PreviousDays ?? Program.ReadValue(
                $"Please enter previous days:",
                DefaultPreviousDays);

            var dateField = this.DateField ?? Program.ReadValue(
                $"Please enter date field for filtering",
                "createdon");
