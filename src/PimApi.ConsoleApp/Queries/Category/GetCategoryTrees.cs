...
namespace PimApi.ConsoleApp.Queries.Category
{
    ...
    public class GetCategoryTrees : IQuery, IQueryWithMessageRenderer, IQueryWithTopSkip, IQueryWithParentId
    {
        ...
        IApiResponseMessageRenderer IQueryWithMessageRenderer.MessageRenderer => CategoryTreeListRenderer.Default;
        ...
        public ApiResponseMessage Execute(HttpClient pimApiClient)
        {
            ...
            var taxonomyTreeId = this.CategoryTaxonomyTreeId ??
            ...
            return pimApiClient.GetAsync(new ODataQuery<CategoryTreeDto>
            {
                Filter = filter,
                Count = true,
                Expand = $"{nameof(CategoryTreeDto.CategoryTaxonomy)}($select=name),"
