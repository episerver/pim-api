...
namespace PimApi.ConsoleApp
{
    internal static class ConsoleQueryExtensions
    {
        ...
        internal static List<IApiResponseMessageRenderer> GetRenderChoices(this IQueryWithMessageRenderer _)
        {
            if (RendererChoices.Count > 0) { return RendererChoices; }

            var rendererType = typeof(UserChoiceRenderer).Assembly.GetTypes()
                .Where(o => typeof(IApiResponseMessageRenderer).IsAssignableFrom(o)
                    && o.IsClass
                    && !o.IsAbstract);

            foreach (var type in rendererType)
            {
                if (typeof(UserChoiceRenderer).IsAssignableFrom(type)) { continue; }
                if (Activator.CreateInstance(type) is not IApiResponseMessageRenderer renderer) { continue; }
        ...
        {
            var id = query.Id ?? Program.ReadValue(
                $"Please enter the ID:",
                Guid.Empty);
        ...
        {
            var id = query.ParentId ?? Program.ReadValue<Guid?>(
        ...
        {
            var skip = query.Skip ?? Program.ReadValue(
                $"Please enter number of records to skip:",
                0);
        ...
        {
            var top = query.Top ?? Program.ReadValue(
                $"Please enter number of records to return:",
                25);
        ...
        {
            if (messageWriter is null) { throw new ArgumentNullException(nameof(messageWriter)); }
            ...
            where TEntity : BaseEntityDtoWithEndpoint
        {
            ...
            {
                messageWriter($" Found: {response.Count.Value:N0} matching {response.Value.FirstOrDefault()?.EntityUrlBase ?? "records"}");
            }
        }
        ...
        internal static string DisplayValue(this object? value)
        {
            ...
            if (value is string s && s.Length == 0) { return "empty string"; }
