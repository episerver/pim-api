...
{
    internal static readonly string[] YesNoChoice = ["yes", "no"];
    ...
    )
    {
        ...
        while (!isStop)
        {
            var request = ReadValue("Please enter query by name or number:", string.Empty)
                .TrimEnd(':');
            ...
            if (!GetQueryByRequest(request, queries, out var query))
            {
            ...
            Console.WriteLine();
            Console.WriteLine(
                $"{query.Value.DisplayName} {(responseSuccess ? "succeeded" : "failed")}!"
            );
            ...
            if (responseSuccess && query!.Value.Query is IQueryWithMessageRenderer writer)
            {
                if (ReadValue("Display content?", string.Empty, YesNoChoice).IsYes())
                {
                    try
                    {
                        await writer.MessageRenderer.Render(
                            response,
                            jsonSerializer,
                    ...
                    {
                        Console.WriteLine(
                            $"Unable to render content using {writer.MessageRenderer.GetType().FullName}!"
                        );
            ...
            if (ReadValue("Display raw response content?", string.Empty, YesNoChoice).IsYes())
            {
                var stringContent = await (
                    await response.GetHttpResponseMessage()
                ).Content.ReadAsStringAsync();
                Console.WriteLine("Response message:");
                Console.WriteLine(stringContent);
                Console.WriteLine();
                Console.WriteLine(
                    $"Response size ({stringContent.Length.Bytes().Humanize(format: "#.##")}): "
                );
                Console.WriteLine();
            }
    ...
    internal static List<QueryDescriptor> GetQueries()
    {
        ...
        {
            if (Activator.CreateInstance(type) is not IQuery intance)
            {
                continue;
            }
    ...
    )
    {
        ...
        query = queries.FirstOrDefault(o =>
            string.Compare(request, o.DisplayName, ignoreCase: true) == 0
        );
    ...
    private static void PrintList(List<QueryDescriptor> queries)
    {
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    $"\t{descriptor.QueryNumber} {descriptor.DisplayName}: {descriptor.Description}"
                );
            }
    ...
    private static readonly HashSet<string> stopwords =
        new(StringComparer.OrdinalIgnoreCase) { "q", "quit", "end", "exit", "stop", };
    ...
    )
    {
        ...
        Console.Write(
            $"{prompt} {GetDefaultValue(defaultValue)} {GetChoiceListDisplay(choiceList)} "
        );
    ...
    private static string GetChoiceListDisplay(ICollection<string>? choices) =>
        choices is null || choices.Count == 0 ? string.Empty : $"({string.Join('|', choices)})";
    ...
    private static async Task ShowGrpcServices(
        IJsonSerializer jsonSerializer,
        string? pimUrlBase,
        string? appKey,
        string? appSecret
            ...
            try
            {
                ...
                switch (selectedItem)
                {
                    case "1":
                        ...
                        Console.WriteLine(
                            $"{jsonSerializer.Serialize(await grpcService.Import(importTemplate, timeZone, RequestData.GetProducts()), null)}\n"
                        );
                        break;
                    case "2":
                        var requestId = ReadValue("Enter request id:", string.Empty);
                        Console.WriteLine(
                            $"{jsonSerializer.Serialize(await grpcService.GetImportStatus(requestId), null)}\n"
                        );
                        break;
                    case "3":
                        ShowGrpcServiceMenu();
