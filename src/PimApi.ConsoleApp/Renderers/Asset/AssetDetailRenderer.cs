...
internal class AssetDetailRenderer : IApiResponseMessageRenderer
{
    ...
    )
    {
        ...
        messageWriter(
            $"{nameof(AssetDto.InternalFileName)} = {dto.InternalFileName.DisplayValue()}"
        );
        ...
        messageWriter(
            $"{nameof(AssetDto.DocumentLanguage)} = {dto.DocumentLanguage.DisplayValue()}"
        );
        messageWriter($"{nameof(AssetDto.FileExtension)} = {dto.FileExtension.DisplayValue()}");
        messageWriter(
            $"{nameof(AssetDto.PreviousInternalFileName)} = {dto.PreviousInternalFileName.DisplayValue()}"
        );
        messageWriter(
            $"{nameof(AssetDto.PreviousFileExtension)} = {dto.PreviousFileExtension.DisplayValue()}"
        );
        ...
        messageWriter(
            $"{nameof(AssetDto.CurrentAssetHash)} = {dto.CurrentAssetHash.DisplayValue()}"
        );
        messageWriter(
            $"{nameof(AssetDto.CurrentVersionNumber)} = {dto.CurrentVersionNumber.DisplayValue()}"
        );
        messageWriter($"{nameof(AssetDto.TagList)} = {dto.TagList.DisplayValue()}");

        if (dto.ProductAssets.Count == 0)
        {
            return;
        }
