...
internal class WebsiteListRenderer : IApiResponseMessageRenderer
{
    ...
    {
        var response = await apiResponseMessage.GetDataAsync<ODataResponseCollection<WebsiteDto>>(
            jsonSerializer
        );
    ...
    internal static void RenderTable(Action<string> messageWriter, ICollection<WebsiteDto> websites)
    {
        ...
        foreach (var entity in websites)
        {
            table.AddRow(
                ...
                entity.Name,
                entity.IscWebsiteId,
                string.Join('|', entity.CategoryTaxonomies.Select(o => o.CategoryTaxonomyId))
            );
        }
